...
//        IntStream.of(elements).boxed().forEach(e -> System.out.print(e + " "));
        System.out.println(trap(new int[]{0, 1, 0, 2, 1, 0, 1, 3, 2, 1, 2, 1}));
    }

    /**
     * 1201. 下一个更大的数 II (lintcode)
     * 中文English
    ...
    public static int trap(int[] height) {
        ...
        // 单调栈： 遇到左边第一个比自己高的数，和右边第一个比自己高的数 ->
        // (Math.min(left, right) - self) * dis(index(left) - index(right))
        for (int i = 0; i < n; i++) {
            while (!stack.isEmpty() && height[stack.peek()] < height[i]) {
                ...
                if (!stack.isEmpty()) {
                    ...
                    ans += (Math.min(height[leftIdx], height[i]) - height[idx]) * (i - leftIdx - 1);
                }
            }
            stack.push(i);
        }
...
//        bstFromPreorder(new int[]{4, 2});
    ...
    private TreeNode helper(int[] preorder, int left, int right) {
        ...
        node.left = helper(preorder, left + 1, mid);
    ...
    @Test
    public void test9() {
//        own.TreeNode<Integer> treeNode = buildTree(new int[]{3, 9, 20, 15, 7}, new int[]{9, 3, 15, 20, 7});
        own.TreeNode<Integer> treeNode = buildTree(new int[]{1, 2}, new int[]{1, 2});
//        own.TreeNode<Integer> treeNode = buildTree(new int[]{3, 1, 2, 4}, new int[]{1, 2, 3, 4});
        printNode(treeNode);
    }

    public own.TreeNode<Integer> buildTree(int[] preorder, int[] inorder) {
        Map<Integer, Integer> indexMap = new HashMap<>();
        for (int i = 0; i < inorder.length; i++) {
            indexMap.put(inorder[i], i);
        }

        return help(0, preorder.length - 1, indexMap, preorder, new PreIndex(0));
    }

    // left and right对应 inorder的元素下标位置
    private own.TreeNode<Integer> help(int left, int right, Map<Integer, Integer> indexMap, int[] preorder, PreIndex preIndex) {
        if (left > right) return null;

        int rootIndex = preIndex.preIndex;
        own.TreeNode<Integer> node = new own.TreeNode(preorder[rootIndex]);
        int mid = indexMap.get(preorder[rootIndex]);

        preIndex.preIndex += 1;

        node.left = help(left, mid - 1, indexMap, preorder, preIndex);
        node.right = help(mid + 1, right, indexMap, preorder, preIndex);
...
//4,2,6,3,1,5
        ...
        subtree = isSubtree(code.deserialize("3,4,5,1,2,#,#,#,#,0"),
                code.deserialize("4,1,2"));
        System.out.println(subtree);

        subtree = isSubtree(code.deserialize("3,4,5,1,2"),
                code.deserialize("4,1,2"));
        System.out.println(subtree);
    }
    ...
    public boolean isSubtree_i(TreeNode s, TreeNode t) {
        if (s == null || t == null) return false;

        // 先序遍历
        String s1 = treeToString(s);
        String t1 = treeToString(t);
...
//        productExceptSelf(new int[]{1, 2, 3, 4});
    ...
    public List<Integer> spiralOrder(int[][] matrix) {
        ...
        while (true) {
            for (int i = l; i <= r; i++) result.add(matrix[u][i]); ////向右移动直到最右
            if (++u > d) break;////重新设定上边界，若上边界大于下边界，则遍历遍历完成，下同
            for (int i = u; i <= d; i++) result.add(matrix[i][r]);//向下
            if (--r < l) break;//重新设定右边界
            for (int i = r; i >= l; i--) result.add(matrix[d][i]); //向左
            if (--d < u) break;//重新设定下边界
            for (int i = d; i >= u; i--) result.add(matrix[i][l]); //向上
...
//        System.out.println(fourSum(new int[]{-3,-2,-1,0,0,1,2,3}, 2));
    ...
    public List<List<Integer>> fourSum(int[] nums, int target) {
        ...
        help(nums, set, new ArrayList<>(), target, 0, target);
    ...
    private void help(int[] nums, Set<List<Integer>> ans, List<Integer> result, int target, int index, int sum) {

        if (result.size() == 4 && sum(result, sum)) {
        ...
        help(nums, ans, result, target - nums[index], index + 1, sum);

        result.remove(result.size() - 1);
        help(nums, ans, result, target - nums[index], index + 1, sum);
    }

    private boolean sum(List<Integer> result, int target) {
        int sum = 0;
        for (int i : result) {
            sum += i;
        }
    ...
    @Test
    public void test14() {
        intArrayPrint(twoSum(new int[]{2, 7, 11, 15}, 9));
    }
    ...
    public int[] twoSum(int[] numbers, int target) {
        ...
        int left = 0, n = numbers.length, right = n - 1, sum = 0;
        int[] ans = new int[2];
        while (left <= right) {
            ...
            if (sum == target) {
                ...
                // 因为已排序，两数和小于目标值，需要将sum调大，
                // 而 num[right] 已经是最大值，所以需要 left ++
                // sum > target ，反之 right ++
                left++;
            } else {
                right--;
    ...
    private void intArrayPrint(int[] arrs) {
        System.out.println(IntStream.of(arrs).boxed().collect(Collectors.toList()).toString());
    }

    /**
     * 653. 两数之和 IV - 输入 BST(leetcode)
     * <p>
     * 给定一个二叉搜索树和一个目标结果，如果 BST 中存在两个元素且它们的和等于给定的目标结果，则返回 true。
     * <p>
     * 因为树的中序遍历（左->根->右）是从小到大的顺序，（右->根->左）是从大到小的顺序，
     * 再结合排序数组的查找方式
     *
     * @param root
     * @param k
    ...
    public List<List<Integer>> threeSum(int[] nums) {
        ...
        Arrays.sort(nums);
        int first = 0, last = 0, n = nums.length, i = 0, sum = 0;
        while (i < n - 2) {
...
//                if (nums[i] * nums[last] > 0) break; // 三数同符号，无解
                ...
                if (sum <= 0) { // 负数过小，first右移
                    while (first < last && nums[first] == nums[++first]) ;// 重复值跳过
                } else {
                    while (first < last && nums[last] == nums[--last]) ; // 重复值跳过
                }
            }
    ...
    public List<List<Integer>> threeSum_ii(int[] nums) {
...
//                if (nums[i] * nums[last] > 0) break; // 三数同符号，无解
                ...
                if (sum == 0) {
                    ...
                    while (first < last && nums[first] == nums[first + 1]) {// 重复值跳过
                        first += 1;
                    }
                    while (first < last && nums[last] == nums[last - 1]) {// 重复值跳过
                        last -= 1;
                    }

                    first += 1;
                    last -= 1;
                } else if (sum <= 0) { // 负数过小，first右移
                    first += 1;
                } else {
                    last -= 1;
                }
     ...
     * @return
     */
    public List<List<Integer>> threeSum_i(int[] nums) {

        // 相对于二数之和，三数之和相当于求 n 次二数之和
        // num : count
        Map<Integer, Integer> map = new HashMap<>();
        for (int num : nums) {
...
//        System.out.println(missingNumber(new int[]{3, 0, 1}));
    ...
    private void findTarget(Map<Integer, Integer> map, int target, Set<List<Integer>> set) {

        for (int num : map.keySet()) {
            if (map.get(num) == 0) continue;
            // target - num == num ;// 表示还需要一个num
            int count = (target - num == num) ? 1 : 0;
            if (map.getOrDefault(target - num, 0) > count) {
    ...
    public int[] findErrorNums(int[] nums) {
        ...
        int n = nums.length, sum = nums[0], temp = 0;

        // 位运算，如果遇到出现两次，结果不会变动
        int dummy = 0, miss = 0;
        for (int i = 1; i < n; i++) {
            temp = sum ^ nums[i];
            if (temp == sum) {// 遇到重复的那个元素
                dummy = nums[i];
                break;
            }
            sum = temp;
        ...
        miss = sum - dummy;
        return new int[]{dummy, miss};
    }

    @Test
    public void test17() {
        intArrayPrint(maxSlidingWindow(new int[]{1, -1}, 1));
        intArrayPrint(maxSlidingWindow(new int[]{1, 3, -1, -3, 5, 3, 6, 7}, 3));
    }
    ...
    public int[] maxSlidingWindow(int[] nums, int k) {
        ...
        // 核心需要处理滑动窗口内的连续最大值。
        // 参考单调栈，可以使用单调递减栈，获取最大值，需要获取栈底的元素，
        // 所以可以使用 双端队列 deque
    ...
    private void outQueue(Deque<Integer> deque, int value) {
        if (!deque.isEmpty() && deque.peek() == value) {
            deque.poll();
        }
    }

    private void inQueue(Deque<Integer> deque, int value) {
